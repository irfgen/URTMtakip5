...
import ParcaTakipListesiModal from './ParcaTakipListesiModal';

const ParcaTakipListeleriYonetModal = ({ open, onClose }) => {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await parcaTakipListeleriService.remove(item.id);
      await load();
    } catch (e) {
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6">Parça Takip Listeleri</Typography>
...
export default ParcaTakipListeleriYonetModal;
