...
class PackageBuilder:
    ...
    def create_download_info(self, tar_path, zip_path):
        ...
        # version.py'den changelog bilgilerini al
        changelog = []
        try:
            from version import RELEASE_NOTES
            # RELEASE_NOTES listesini doğrudan kullan
            changelog = RELEASE_NOTES
        except Exception:
            # Fallback changelog
            changelog = [
                f'v{self.version} - {datetime.now().strftime("%Y-%m-%d")}:',
                '- En son sürüm güncellemeleri',
            ]
        ...
        download_info = {
            ...
            'build_number': self.version_full.split('.')[-1] if '.' in self.version_full else '',
            'download_url': f'downloads/URTM_DizinTarama_Client_v{self.version}.zip',
            'file_size': f'{zip_path.stat().st_size // 1024}KB',
            'changelog': changelog,
            'requirements': [
                'Python 3.8+',
                'Windows 10/11',
        ...
        print(f"   ✓ {download_file.name} (kök dizin)")
...
"""
    ...
    def build(self):
        ...
        try:
            ...
            # 5. İndirme bilgileri oluştur ve paket dizinine de kopyala
            download_info = self.create_download_info(tar_path, zip_path)

            # Paket dizinine de kopyala
            package_json = package_dir / 'LATEST_RELEASE.json'
            with open(package_json, 'w', encoding='utf-8') as f:
                json.dump(download_info, f, indent=2, ensure_ascii=False)
            print(f"   ✓ {package_json.name} (paket dizini)")
