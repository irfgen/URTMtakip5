...
class DizinTaramaClient:
    ...
    def create_gui(self):
        ...
        if DATABASE_INTEGRATION:
            ...
            # Montajları Seç butonu
            self.select_assemblies_button = ttk.Button(selection_buttons_frame, text="🔧 Montajları Seç",
                                                       command=self.select_assemblies, state="disabled")
            self.select_assemblies_button.grid(row=0, column=4, padx=(0, 5))
            ...
            self.show_details_button.grid(row=0, column=5, padx=(20, 0))
        ...
        if DATABASE_INTEGRATION:
            columns = ('☑', 'Parça Adı', '3D (.sldprt)', 'Çizim (.slddrw)', 'Montaj (.sldasm)', 'PDF (.pdf)', 'Durum', 'DB Durumu')
        else:
            columns = ('Parça Adı', '3D (.sldprt)', 'Çizim (.slddrw)', 'Montaj (.sldasm)', 'PDF (.pdf)', 'Durum')
    ...
    def send_enhanced_scan_results(self, directory, scan_data):
        ...
        try:
            ...
            # Parçaları ekle
            for part_name, part_data in parts.items():
                # Aynı isimli montaj dosyası var mı kontrol et
                part_assembly_files = []
                if part_name in assemblies:
                    part_assembly_files = assemblies[part_name]['sldasm']
                ...
                parca = {
                    ...
                    'pdf': part_data['pdf'],
                    'sldasm': part_assembly_files,  # Aynı isimli montaj dosyası
                    'has3D': len(part_data['sldprt']) > 0,
                    'hasDrawing': len(part_data['slddrw']) > 0,
                    'hasPDF': len(part_data['pdf']) > 0,
                    'hasAssembly': len(part_assembly_files) > 0,  # Montaj dosyası var mı?
                    'level': part_data['level'],
                    'path': part_data['path'],
                    'parentAssembly': part_data['parent_assembly'],
                    'toplamDosya': len(part_data['sldprt']) + len(part_data['slddrw']) + len(part_data['pdf']) + len(part_assembly_files)
                }
                parca_listesi.append(parca)
        ...
        for parca in data.get('parcaListesi', []):
            ...
            if DATABASE_INTEGRATION and self.selection_manager:
                ...
                part_data = {
                    ...
                    'hasPDF': parca['hasPDF'],
                    'hasAssembly': parca.get('hasAssembly', False),  # Montaj bilgisi
                    'sldprt_files': parca.get('sldprt', []),
                    'slddrw_files': parca.get('slddrw', []),
                    'sldasm_files': parca.get('sldasm', []),  # Montaj dosyaları
                ...
                values = (
                    ...
                    len(parca['sldprt']),
                    len(parca['slddrw']),
                    len(parca.get('sldasm', [])),  # Montaj dosyaları
                    len(parca['pdf']),
                    durum,
...
Tarama Zamanı: {data.get('taramaZamani', 'Bilinmiyor')}"""
    ...
    def enable_selection_controls(self):
        ...
        self.select_assemblies_button.config(state="normal")
    ...
    def disable_selection_controls(self):
        ...
        self.select_assemblies_button.config(state="disabled")
    ...
    def select_assemblies(self):
        """Montaj dosyası (.sldasm) olan parçaları seç"""
        if not self.selection_manager:
            self.logger.warning("SelectionManager mevcut değil")
            return

        # Parça listesini self.last_scan_data'dan al
        selected_count = 0

        if not hasattr(self, 'last_scan_data') or not self.last_scan_data:
            self.logger.warning("Tarama verisi bulunamadı")
            return

        parts_list = self.last_scan_data.get('parcaListesi', [])
        self.logger.info(f"Toplam {len(parts_list)} parça analiz ediliyor...")

        # Her parçayı kontrol et
        for part in parts_list:
            part_name = part.get('parcaAdi', '')
            sldasm_files = part.get('sldasm', [])
            has_assembly_flag = part.get('hasAssembly', False)

            # .sldasm dosyası var mı kontrol et (hasAssembly veya sldasm listesi dolu mu)
            has_assembly = has_assembly_flag or (len(sldasm_files) > 0)

            if has_assembly:
                self.logger.debug(f"Montaj bulunan parça: {part_name}, hasAssembly={has_assembly_flag}, sldasm count={len(sldasm_files)}")
                # Parçayı seç
                if self.selection_manager.set_part_selection(part_name, True):
                    selected_count += 1
                else:
                    self.logger.warning(f"Parça seçilemedi: {part_name}")

        self.logger.info(f"Montaj dosyası olan {selected_count} parça seçildi")
