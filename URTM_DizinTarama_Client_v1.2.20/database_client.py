...
class DatabaseClient:
    ...
    def save_parts_to_database(self, parts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            url = f"{self.api_base}/save-parts"
            self.logger.info(f"📤 API URL: {url}")
            for i, part in enumerate(parts):
                self.logger.info(f"  Part {i+1}: {part.get('parcaKodu', part.get('fileName', 'UNKNOWN'))}")
                self.logger.info(f"    - sldprt_yolu: {part.get('sldprt_yolu', 'YOK')[:100] if part.get('sldprt_yolu') else 'YOK'}")
                self.logger.info(f"    - slddrw_yolu: {part.get('slddrw_yolu', 'YOK')[:100] if part.get('slddrw_yolu') else 'YOK'}")
                self.logger.info(f"    - sldasm_yolu: {part.get('sldasm_yolu', 'YOK')[:100] if part.get('sldasm_yolu') else 'YOK'}")
                self.logger.info(f"    - pdf_yolu: {part.get('pdf_yolu', 'YOK')[:100] if part.get('pdf_yolu') else 'YOK'}")
