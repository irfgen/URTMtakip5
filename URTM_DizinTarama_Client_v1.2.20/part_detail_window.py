...
class PartDetailWindow:
    ...
    def combine_part_data(self, scan_data: Dict, api_data: Dict) -> Dict[str, Any]:
        ...
        Args:
            scan_data: Dosya tarama verisi (SelectionManager'dan gelen)
        ...
        # Dosya yollarını normalize et (tutarlı isimlendirme için)
        # SelectionManager'dan gelen veri _files suffix ile geliyor
        combined['sldprt_files'] = scan_data.get('sldprt_files', scan_data.get('sldprt', []))
        combined['slddrw_files'] = scan_data.get('slddrw_files', scan_data.get('slddrw', []))
        combined['sldasm_files'] = scan_data.get('sldasm_files', scan_data.get('sldasm', []))
        combined['pdf_files'] = scan_data.get('pdf_files', scan_data.get('pdf', []))
    ...
    def create_cad_files_section(self, parent: ttk.LabelFrame, part_data: Dict) -> None:
        ...
        self.logger.info(f"CAD Files - sldasm_files: {part_data.get('sldasm_files', [])}")
        ...
        # Montaj (.sldasm)
        sldasm_files = part_data.get('sldasm_files', [])
        self.logger.info(f"Assembly files found: {sldasm_files}")

        if sldasm_files and len(sldasm_files) > 0:
            file_path = sldasm_files[0]
            file_name = os.path.basename(file_path)
            ...
            ttk.Label(file_frame, text="├─ Montaj (.sldasm): ✅", font=('Consolas', 9)).pack(side='left')
            ...
            file_link.pack(side='left', padx=(5, 0))
            self.logger.debug(f"✅ Added Assembly button: {file_name}")
        else:
            # Montaj dosyası yoksa normal (uyarı değil, çünkü her parçada montaj yok)
            error_text = "├─ Montaj (.sldasm): ⚠️ Montaj dosyası yok"
            self.logger.info(error_text)
            ttk.Label(cad_frame, text=error_text, font=('Consolas', 9), foreground='gray').pack(anchor='w', pady=2)
        ...
        # Debug: Toplam durum
        total_files = len(sldprt_files) + len(slddrw_files) + len(sldasm_files) + len(pdf_files)
        self.logger.info(f"CAD Files Summary - Total files: {total_files} (3D: {len(sldprt_files)}, 2D: {len(slddrw_files)}, ASM: {len(sldasm_files)}, PDF: {len(pdf_files)})")
    ...
    def save_parts_to_database(self) -> None:
        ...
        try:
            ...
            for part in self.selected_parts:
                ...
                sldasm_files = cad_data.get('sldasm_files', [])
