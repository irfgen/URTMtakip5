...
REM Read FreeCAD path from config
for /f "tokens=2 delims== " %%A in ('findstr "^path" config.ini 2^>nul') do (
    set "FREECAD_PATH=%%A"
)

REM Read Python path from config
for /f "tokens=2 delims== " %%A in ('findstr "^python_path" config.ini 2^>nul') do (
...
REM Check if running in console or double-clicked
echo %cmdcmdline% | find /i "%~0" >nul
if not errorlevel 1 (
    REM Script was double-clicked, run GUI without console
    echo %GREEN%[GUI] GUI modu - konsol gizleniyor...%RESET%
    echo.
    
    REM Use pythonw to run without console window
    set "PYTHON_EXEC=!FREECAD_PYTHON!"
    echo !PYTHON_EXEC! | find "python.exe" >nul
    if !errorlevel! == 0 (
        set "PYTHON_EXEC=!PYTHON_EXEC:python.exe=pythonw.exe!"
    )
    
    "!PYTHON_EXEC!" start_gui.py
    
) else (
    REM Script was run from command line, keep console
